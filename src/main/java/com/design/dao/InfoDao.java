...
public interface InfoDao {
    ...
    @Select("SELECT  Book_info.name,count(*) num from Borrow,Book_info where DATE(borrow_time)>DATE_SUB(CURDATE(), INTERVAL 1 YEAR) and Book_info.id=Borrow.id GROUP BY Book_info.name,Book_info.pub,Book_info.time  ORDER BY num DESC limit 5")
    public List<Info> getYearRankBookBorrow();
    @Select("SELECT  Book_info.name,count(*) num from Borrow,Book_info where DATE(borrow_time)>DATE_SUB(CURDATE(), INTERVAL 1 MONTH) and Book_info.id=Borrow.id GROUP BY Book_info.name,Book_info.pub,Book_info.time  ORDER BY num DESC limit 5")
    public List<Info> getMonthRankBookBorrow();
    @Select("SELECT  Book_info.name,count(*) num from Borrow,Book_info where DATE(borrow_time)>DATE_SUB(CURDATE(), INTERVAL 1 WEEK) and Book_info.id=Borrow.id GROUP BY Book_info.name,Book_info.pub,Book_info.time  ORDER BY num DESC limit 5")
    public List<Info> getWeekRankBookBorrow();
    @Select("SELECT  Book_info.name,count(*) num from Borrow,Book_info where DATE(borrow_time)=CURRENT_DATE and Book_info.id=Borrow.id GROUP BY Book_info.name,Book_info.pub,Book_info.time  ORDER BY num DESC limit 5")
